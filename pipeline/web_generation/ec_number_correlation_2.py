...
def analyze_ec_levels(mapped_data, output_dir):
    """
    Analyze and create visualizations for EC levels.
    """
...
def additional_visualizations(mapped_data, output_dir):
    """
    Create additional visualizations to illustrate EC level distributions.
    """
    # Ensure the EC levels are numeric
    for level in range(1, 5):
        ec_level = f'ec_level_{level}'
        mapped_data[ec_level] = pd.to_numeric(mapped_data[ec_level], errors='coerce')

        # Stacked bar plot
        plt.figure(figsize=(14, 7))
        sns.histplot(data=mapped_data, x=ec_level, hue='evodex_type', multiple='stack', palette='viridis', shrink=0.8)
        plt.title(f'Stacked Bar Plot of EC Level {level} by EVODEX Type')
        plt.xlabel(f'EC Level {level}')
        plt.ylabel('Count')
        plt.savefig(os.path.join(output_dir, f'stacked_bar_ec_level_{level}.png'))
        
        # Violin plot
        plt.figure(figsize=(14, 7))
        sns.violinplot(x='evodex_type', y=ec_level, data=mapped_data, palette='Set2')
        plt.title(f'Violin Plot of EC Level {level} Distribution by EVODEX Type')
        plt.xlabel('EVODEX Type')
        plt.ylabel(f'EC Level {level}')
        plt.savefig(os.path.join(output_dir, f'violin_plot_ec_level_{level}.png'))
...
def main():
    ...
    additional_visualizations(mapped_data, output_dir)
